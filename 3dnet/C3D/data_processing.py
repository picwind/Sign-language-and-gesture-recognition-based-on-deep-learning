...
def get_video_indices(filename):
    ...
    #Shuffle data
    lines = list(lines)
    video_indices = [i for i in range(len(lines))]
...
def prepare_con_depth_data(video_path, CLIP_LENGTH, crop_size, channel_num, is_training):
...
def get_batches(filename, num_classes, CLIP_LENGTH, batch_index, video_indices, batch_size=20, crop_size=112, channel_num=3):
    lines = open(filename, 'r')
    clips = []
    labels = []
    lines = list(lines)
    for i in video_indices[batch_index: batch_index + batch_size]:
        ...
        dirname = line[0]
        label = line[1]
        i_clip = prepare_con_depth_data(dirname, CLIP_LENGTH, crop_size, channel_num, True) #将171x128x16的视频转换成112x112x16的大小
        clips.append(i_clip)
        labels.append(int(label))
    clips = np.array(clips).astype(np.float32)
    labels = np.array(labels).astype(np.int64) - 1
    #one-hot编码
